...
county_precinct_totals <- combined_primary %>%
  ...
  summarize(total_governors_race_votes = sum(early_votes + election_night_votes + mail_in_ballot_1_votes + mail_in_ballot_2_votes, na.rm = TRUE), .groups = 'drop')
...
top_precinct_location <- left_join(top_precinct, precinct_voting_locations, by = c( "county" = "county_code", 
